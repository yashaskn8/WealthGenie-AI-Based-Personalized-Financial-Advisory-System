...
| Strict item 5: UTF-8 BOM/mojibake cleanup | Y | BOM/mojibake corruption across tracked text files | Cleaned editable tracked files and verified mojibake clean; left nginx.conf unchanged due explicit no-touch deployment constraint | BOM scan now reports only reactapp/nginx.conf; mojibake scan prints MOJIBAKE_SCAN_CLEAN |
| Strict item 6: Frontend test coverage | Y | React app had no real executable unit/component tests | Added Vitest and tests for taxCalculator, sipCalculator, postTaxEngine, portfolioValidation, recommendationEngine, and fallback banner rendering/dismissal | npm test in reactapp passed: 6 files, 12 tests |
| Strict item 7: Backend test coverage | Y | Backend services/routes lacked required coverage | Added service and route coverage for LLM services, prompt builder, constants, market data, ML client, calculators, profiler, and protected/public routes | npm test in server passed: 30 tests |
| Strict item 8: Dead npm dependencies | Y | html2canvas/html2pdf.js were direct dependencies without source imports | Removed direct dependencies and corrected README PDF claim to jsPDF; retained jspdf because HealthScoreScreen uses it | rg shows no source/package direct html2pdf/html2canvas; package-lock hit is transitive optional html2canvas from jspdf |
| Strict item 9: Env examples | Y | Missing env templates/entries for server, ML, and React | Added CORS_ORIGINS, ml-service/.env.example, reactapp/.env.example, and ensured GROQ_API_KEY is documented | Env scan shows SERVER_DOC/ML_DOC/REACT_DOC all True |
| Strict item 10: Fiscal-year tax slabs | Y | Tax slabs were hardcoded without FY selection | Added CURRENT_FY, TAX_SLABS_BY_FY, getTaxSlabsForFY, FY2026-27 TODO duplicate, and FY selection threading | node --check taxEngine passed; npm test includes FY slab selection test |
| Strict item 11: Bare console.log cleanup | Y | Runtime debug console.log statements remained outside CLI/training output | Converted or removed runtime bare console.log statements, preserving allowed seed/train CLI output | rg console.log excluding seedInstruments/train returned no matches; npm test in server passed |