...
test('mlClient posts to enriched endpoint and falls back on service failure', async (t) => {
  ...
  axios.get = async () => ({ data: { status: 'ok' } });
  t.after(() => { axios.post = originalPost; axios.get = originalGet; console.warn = originalWarn; });

  const prediction = await getMLPrediction({ age: 40, annual_income: 1200000, monthly_savings: 25000, risk_category: 'Moderate' });
  const health = await checkMLHealth();
  axios.post = async () => { throw new Error('offline'); };
  const fallback = await getMLPrediction({ age: 60, annual_income: 2200000, monthly_savings: 50000, risk_category: 'Conservative' });
